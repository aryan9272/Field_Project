...
<script>
    ...
    // --- Filtering Logic (Contains the Fix) ---
    function filterItems() {
        ...
        if (visibleItemCards.length === 0 && itemCards.length > 0) {
            ...
            if (!noItemsMessage) {
                // If the message is NOT present, create and APPEND it (DO NOT use innerHTML).
                const msgDiv = document.createElement('div');
                msgDiv.id = 'noItemsMessage';
                // Use the same classes as the PHP version for consistency
                ...
                // IMPORTANT: Use appendChild() to prevent overwriting/deleting itemCards.
                itemGrid.appendChild(msgDiv);
            }
        } else if (noItemsMessage) {
             // Case 2: Items are now visible (filter changed back), so remove the message.
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.addEventListener('click', (event) => {
            ...
            document.querySelectorAll('.modal').forEach(modal => {
                ...
                if (event.target === modal && modal.id !== 'profileModal') {
